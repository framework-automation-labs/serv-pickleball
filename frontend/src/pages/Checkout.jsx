...
export default function Checkout() {
  ...
  const { date, bookings, totalHours, totalPrice, fullName, phone } = state
  ...
  return (
    <div className="min-h-screen bg-mist px-6 py-10">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-line p-5 mb-4 space-y-2 text-sm">
            ...
            {bookings.map((b, i) => (
              <div key={i} className="flex justify-between">
                ...
                <span className="font-semibold text-ink">
                  {formatHour(b.startHour)} – {formatHour(b.endHour)} ({b.hours}h)
                </span>
              </div>
            ))}
          </div>
        ...
        <button
          ...
          className="w-full bg-spark text-white font-display font-semibold py-3.5 rounded-full hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50"
        >
